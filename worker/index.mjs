...
import { runSplit, runExtract } from "./jobs.mjs";

const POLL_MS = 3000;
const JOB_TIMEOUT_MS = 8 * 60 * 1000; // 8분 넘게 매달리면 에러 처리

// 우선순위: split(사용자가 대기 중인 첫 분할) → extract.
const TYPES = ["split", "extract"];

async function runJob(job) {
  const fn = job.type === "split" ? runSplit : runExtract;
...
async function tick() {
  ...
  try {
    ...
    await failStep(job.projectId, msg);
  }
}

console.log("[worker] BUILD = m1-split-v1");
console.log("[worker] 시작 — jobq:split / jobq:extract 폴링 중…");
for (;;) {
  await tick();
