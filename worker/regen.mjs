...
export function buildRegenPrompt(scene, project) {
  const cut = scene.cut ?? {};
  const parts = [];
  if (project.stylePrompt) parts.push(String(project.stylePrompt).trim());
  const content = String(cut.promptDraft || cut.description || "").trim();
  parts.push(content || "Redraw this comic panel faithfully as a clean illustration.");
  const neg = String(project.negativePrompt || "").trim();
  let prompt = parts.filter(Boolean).join(". ");
  if (neg) prompt += `. Do not include: ${neg}.`;
