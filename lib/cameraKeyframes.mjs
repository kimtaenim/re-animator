// ============================================================================
// re-animator — 카메라워크 수식 모듈 (스펙 §2, 단일 소스)
// ----------------------------------------------------------------------------
 ...
 * @property {number} [accel_hold]               ★느린 구간 비율(0~0.9). 0.5=앞 절반 느리게, 뒤 절반에서 몰아침
 * @property {number} [accel_hold_creep]         느린 구간에서 소화할 이동량(0~0.6, 기본 0.15). 0=완전 정지
...
export function buildKeyframeTable(cw, opts = {}) {
  ...
  for (let i = 0; i < frames; i++) {
    ...
    //   "50% 정지 / 50% 줌" 처럼 구간을 사람이 직접 나눌 수 있어야 연출이 된다.
    // ★앞 구간은 '완전 정지' 가 아니라 '아주 느리게 기어감' 이다(사용자 지정) — 멈춰 있으면
    //   정지컷처럼 보인다. hold 구간에서 전체 이동의 creep 만큼(기본 15%)을 등속으로 소화하고,
    //   나머지(85%)를 뒤 구간에서 가속으로 몰아친다.
    const accel = Number(cw.zoom_accel) || 0;
    const hold = clamp(Number(cw.accel_hold) || 0, 0, 0.9);
    const creep = clamp(cw.accel_hold_creep == null ? 0.15 : Number(cw.accel_hold_creep), 0, 0.6);
    let p;
    if (hold > 0) {
      p =
        off <= hold
          ? (off / hold) * creep // 느린 크립(등속)
          : creep + (1 - creep) * (accel > 0 ? Math.pow((off - hold) / (1 - hold), 1 + accel) : ease(easing, (off - hold) / (1 - hold)));
    } else {
      p = accel > 0 ? Math.pow(off, 1 + accel) : ease(easing, off);
    }
    ...
    if (layer === "A" || layer === "C") {
      ...
      // ★측면 시차 — 배경은 드리프트·셰이크의 중심 이동을 40%만 따라간다(가까운 것이 더 크게 움직여야 깊이). 스케일 시차의 횡축판.
      const [bcx, bcy] = clampCenter(0.5 + (cx - 0.5) * 0.4, 0.5 + (cy - 0.5) * 0.4, bgScale);
...
//   값은 cleanCameraWork 클램프 안(zoom ±15·drift ±300·start_zoom 1~3). 슬라이더로 더 줄일 수 있음.
export const CAMERA_PRESETS = {
  ...
  //   "옆으로 이동해서 화면이 비뚤어진다"). 가로 이동이 필요하면 팬 프리셋이나 드리프트 슬라이더로.
  ...
  parallax_push: { zoom_rate_pct_per_s: 6.0, zoom_accel: 1, bg_scale_delta_pct_per_s: -4.0, drift_px_per_s: { x: 10, y: 0 }, easing: "easeInOut" },
  ...
  vertigo: { zoom_rate_pct_per_s: 2, zoom_accel: 2, start_zoom: 1.45, bg_scale_delta_pct_per_s: -14, easing: "easeInOut" }, // 달리줌 — accel 2(off^3): 전 구간 이동이 보이며 후반 가속(6은 off^7이라 앞 85%가 정지로 보였다·사용자 보고)
  orbit: {}, // I2V 위임
};
