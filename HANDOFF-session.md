...
# ★★★★★ 지금 상태 (2026-08-02) — 이어받는 세션은 이 블록부터 읽어라

**배포 HEAD = `cb82c0b`** · **워커 BUILD = `matte-v32`**(로그 첫 줄로 배포 확인, 커밋마다 갱신).
...
2. **인터페이스 재편(대공사)**: 1·2단계는 지금처럼 '단계 우선', **3·4·5단계는 '컷 우선'** —
   한 컷 카드 안에 재생성·영상/더빙·카메라 3개를 모아 한자리에서 조정. 6단계는 시퀀스별 확인 후 합성.
   ★안전한 순서: (a) 각 단계의 컷 카드를 **함수로 추출**(동작 무변경, tsc/build 로 확인) →
   (b) 그 셋을 한 카드에 합친 새 뷰 추가 → (c) 기존 탭은 남겨두고 전환. Studio.tsx 는 5000줄이라
   한 번에 뜯으면 반드시 깨진다(과거 사고). 슬라이스마다 커밋.
