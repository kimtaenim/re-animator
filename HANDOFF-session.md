...
# ★★★★ 지금 상태 (2026-07-24) — 이어받는 세션은 이 블록만 먼저 읽어라
...
5. **더빙** — 안 되면 **정확한 증상**(소리 안 생김/엉뚱한 언어/에러 문구)이 필요. 워커 로그 `더빙 완료: 생성 N개…`.
6. **효과음·BGM** — 합성 결과에 실제로 들리는지. BGM 은 5단계에서 업로드.
7. **언어별 출력** — 5단계 `🌐 언어별 최종 출력`에서 "일본어판 합성" → 받기.
